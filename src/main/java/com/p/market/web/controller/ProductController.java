...
public class ProductController {
    @Autowired
    private ProductService productService; //Inyectamos el servicio de productos
    //Metodos
    ...
    public ResponseEntity<Product> getProduct(@PathVariable("id") int productId) {
        return productService.getProduct(productId)
                ...
                .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));//Obtenemos el producto
    }

    @GetMapping("/category/{categoryId}")
    public ResponseEntity<List<Product>> getByGategory(int categoryId) {
